    ...
    private static class InnerFragmentManager {
        ...
        public Fragment getFrag (int position) {
            ...
            if (frag_db.get(pos_) == null){
                switch (position+1) {
                    case Constants.ALL:
                        frag_db.put(pos_, RefreshableMockFragment.getInstance(AppURL.GET_ORDER_LIST_ALL, tokenKey));
                        break;
                    case Constants.WAITINGPAY:
                        frag_db.put(pos_, RefreshableMockFragment.getInstance(AppURL.GET_ORDER_LIST_WAITING_FOR_PAY, tokenKey));
                        break;
                    case Constants.WAITINGRECEIVING:
                        frag_db.put(pos_, RefreshableMockFragment.getInstance(AppURL.GET_ORDER_LIST_WAITING_FOR_RECEIVE, tokenKey));
                        break;
                    case Constants.WAITINGSENDING:
                        frag_db.put(pos_, RefreshableMockFragment.getInstance(AppURL.GET_ORDER_LIST_WAITING_FOR_SEND, tokenKey));
                        break;
                }
            }
